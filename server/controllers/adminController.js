...
// @desc   Get all reviews, optionally only flagged
// @route  GET /api/admin/reviews?flagged=true
...
// @desc   Remove a review (moderation)
...
export const removeReview = asyncHandler(async (req, res) => {
  ...
  await Review.syncRestaurantRating(restaurantId);

  res.json({ success: true, message: "Review removed" });
});

// @desc   Unflag a review (approve it after moderation)
...
// @desc   Get dishes with unverified nutrition
// @route  GET /api/admin/dishes?verified=false
export const getDishes = asyncHandler(async (req, res) => {
  const filter = {};
  if (req.query.verified === "false") filter.nutritionVerified = false;
  if (req.query.verified === "true") filter.nutritionVerified = true;
...
// @desc   Verify or reject a dish's nutritional data
// @route  PATCH /api/admin/dishes/:id/verify
export const verifyDishNutrition = asyncHandler(async (req, res) => {
  ...
  dish.nutritionVerified = req.body.verified !== false;
  await dish.save();

  res.json({
    success: true,
    message: dish.nutritionVerified ? "Nutrition data verified" : "Verification removed",
    nutritionVerified: dish.nutritionVerified,
  });
});